...
// NAV items are either flat — { key, href, text, icon, badge?, roles? } —
...
//
// `roles` (optional array of 'admin' | 'internal' | 'client') is read at
// build time only to stamp a `data-roles` attribute on the rendered link —
// actual hiding happens client-side in src/lib/auth-guard.js once we know
// who's logged in. Omit `roles` to show the item to everyone.
//
// SMA-app: trimmed from the full Gentelella demo nav down to what the
// product actually uses. Pages still exist under production/ for any of
// the removed demo sections (charts, kanban, e-commerce, etc.) — re-add
// here if a future feature needs them.
export const NAV = [
  {
    label: 'Utama',
    items: [
...
function renderNavItem(item, activeKey) {
  ...
  return `
    <a class="nav-link${a ? ' active' : ''}" href="${item.href}"${a ? ' aria-current="page"' : ''}${rolesAttr}>
      ${ICONS[item.icon] || ''}
      <span class="nav-text">${item.text}</span>
      ${item.badge ? `<span class="badge ${item.badge.cls}">${item.badge.text}</span>` : ''}
    </a>
    ...
    <aside class="sidebar" aria-label="Primary navigation">
      ...
      <div class="sidebar-footer">
        <div class="sidebar-user">
          <div class="avatar" id="sidebar-user-avatar">·<span class="online"></span></div>
          <div class="sidebar-user-info">
            <div class="name" id="sidebar-user-name">Memuat…</div>
